...
const PROCESSING_BATCH_SIZE = 10; // Number of chunks to process in each batch for the Edge Function (reduced for Free plan limits)
...
async function ingestData() {
    try {
        ...
        if (allChunks.length === 0) {
            console.log('⚠️  No chunks generated from any files');
            return;
        }
        ...
        if (successCount > 0) {
            console.log(`\n🔍 Verifying data in ${EMBEDDINGS_TABLE_NAME}...`);
